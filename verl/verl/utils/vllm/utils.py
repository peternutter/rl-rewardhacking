    ...
    def hijack():
        def hijack__load_adapter(self, lora_request: TensorLoRARequest) -> LoRAModel:
            ...
            try:
                ...
                # Safe defaults for attributes that may not exist in all vllm versions
                ...
                # Detect vllm API version
                import inspect
                _new_api = "model_vocab_size" in inspect.signature(self._lora_model_cls.from_lora_tensors).parameters
            ...
            if getattr(lora, "extra_vocab_size", 0) > extra_vocab:
                raise ValueError(
                    f"LoRA added vocab size {getattr(lora, 'extra_vocab_size', 0)} is greater than "
                    f"lora_extra_vocab_size {extra_vocab}."
                )
            return lora
